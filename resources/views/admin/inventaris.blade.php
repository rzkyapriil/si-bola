...
<div id="tambah_modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
			...
			<div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
					...
					<form method="POST" action="{{route('inventaris.store')}}" enctype="multipart/form-data" class="p-4 md:p-5 flex flex-col gap-4">
						...
						<div>
							...
							<input type="number" id="harga" name="harga" placeholder="Masukkan harga" class="block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
  ...
  <div class="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0 mb-6">
		<h1 class="text-2xl font-bold w-fit md:w-full">Inventaris</h1>

		<div class="flex w-full">
			<form method="GET" action="{{route('inventaris.index')}}" class="flex w-full mx-auto">
		...
		<table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
				...
				<tbody>
						...
						<tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
							...
							<td class="px-6 py-4">
								<div class="uppercase">{{$inventory->status}}</div>
							</td>
							<td class="flex px-6 py-4 space-x-2">
								<a href="{{route('inventaris.edit', $inventory->id)}}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
								<form method="POST" action="{{route('inventaris.destroy', $inventory->id)}}">
									@csrf @method('delete')
	...
	@include('components.paginator', ['data' => $inventories])
@endsection